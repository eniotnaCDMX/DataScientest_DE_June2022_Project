...
def update_tab(D_country, D_rate,start_date, end_date):
    ...
    tab = df.groupby('sentiment').count()['Content2'].reset_index(name='Count').sort_values(by='Count',ascending=False).to_dict("records")
    return tab
...
def update_tab2(D_country, D_rate,start_date, end_date):
    ...
    all_words=[]
    for i in range(len(df['Content2'])):
        a=df['Content2'].reset_index()['Content2'][i]
        for i in a:
            all_words.append(i)
    all_words=pd.Series(np.array(all_words))
